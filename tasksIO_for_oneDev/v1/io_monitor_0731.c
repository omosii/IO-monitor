...
static inline struct task_io_stats* find_node(int target, struct hlist_head *task_io_stats_hash) {
    int key = target % IO_STATS_HASH_SIZE; // 简单哈希函数
    struct task_io_stats *entry;
    // struct task_io_stats res;
...
static int io_monitor_show(struct seq_file *m, void *v)
{
    ...
    clear_hlist_head(); // 清理上次哈希表中的所有条目
    init_my_hlist_head(); // 清空旧的并初始化新的哈希表
    ...
    for_each_process(task) {
        ...
        // 如果没有 I/O 操作，则跳过该任务
        if(!first_stats->read_bytes && !first_stats->write_bytes) {
            kfree(first_stats); // 释放当前统计数据
            continue; 
        }
    ...
    u64 be_ns = ktime_get_ns();
    u64 ed_ns = be_ns + SLEEP_MILLISECONDS * NSEC_PER_MSEC;
    while (ktime_get_ns() < ed_ns) {
        cpu_relax();
    } 
    ...
    for_each_process(task) {
        ...
        // 查找历史节点
        spin_lock(&io_stats_hash_lock);  // 获取锁
        struct task_io_stats *tmp = (find_node(second_stats->pid, task_io_stats_hash)); 
        struct task_io_stats stats_last_inst = *tmp;
        struct task_io_stats* stats_last = &stats_last_inst;
        spin_unlock(&io_stats_hash_lock);
        if(tmp == NULL) {
            kfree(second_stats); // 释放当前统计数据
            continue;
        }

        // 获取新的IO统计
        get_dev_io_stats(task, &second_stats->read_bytes, &second_stats->write_bytes, &second_ns);
        if(!second_stats->read_bytes && !second_stats->write_bytes) {
            kfree(second_stats); // 释放当前统计数据
            continue;
        }
        ...
        // 安全的时间差计算
        u64 read_delta_ns = (second_stats->record_time_ns - stats_last->record_time_ns);
        if (read_delta_ns <= 0){
            kfree(second_stats); // 释放当前统计数据
            continue;
        }
        u64 read_delta_ms = div_u64(read_delta_ns, 1000000); 

        // 安全的速率计算
        if (read_delta_ms > 0) {
            second_stats->read_Bpms = div_u64((second_stats->read_bytes - stats_last->read_bytes)*ZOOM_FACTOR, read_delta_ms);
            second_stats->write_Bpms = div_u64((second_stats->write_bytes - stats_last->write_bytes)*ZOOM_FACTOR, read_delta_ms);
        } else {
            second_stats->read_Bpms = 0;
            second_stats->write_Bpms = 0;
        }

        // stats.comm 与 stats.parent_comm获取
        strscpy(second_stats->comm, task->comm, TASK_COMM_LEN); 
        if (task->real_parent) {    
            strscpy(second_stats->parent_comm, task->real_parent->comm, TASK_COMM_LEN);
        } else {
        ...
        seq_printf(m, "%-16s %-8d %-16s %-8d %-12lu %-12lu\n",
            ...
            second_stats->ppid,
            second_stats->read_Bpms,
            second_stats->write_Bpms);
